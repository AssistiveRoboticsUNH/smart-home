  ...
  const sensor_msgs::Image::ConstPtr& msg)
{
  ...
  {
    normals_estimator_ = new cv::rgbd::RgbdNormals(cv_ptr->image.rows,
                                             cv_ptr->image.cols,
                                             cv_ptr->image.depth(),
                                             K_);
  }
  ...
  {
    /// plane_estimator_ = cv::Algorithm::create<cv::rgbd::RgbdPlane>("RGBD.RgbdPlane");
    plane_estimator_ = new cv::rgbd::RgbdPlane(); 
    // plane_estimator_ = cv::xfeatures2d::SIFT::create();
    // plane_estimator_ = cv::xfeatures2d::SURF::create();
