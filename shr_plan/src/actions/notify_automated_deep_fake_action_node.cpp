...
class NotifyAutomatedDeepFake : public plansys2::ActionExecutorClient {
public:
    NotifyAutomatedDeepFake(const std::string &action, const std::string &script_name, int wait_time,
                    const std::string &sensor_topic, const std::string  &voice_name)
        ...
        sensor_sub_ = create_subscription<std_msgs::msg::Bool>(sensor_topic, 10,
                                                               std::bind(&NotifyAutomatedDeepFake::callback, this,
                                                                         std::placeholders::_1));
    }
    ...
    on_activate(const rclcpp_lifecycle::State &previous_state) {
        ...
        read_action_client_ =
                rclcpp_action::create_client<shr_msgs::action::DeepFakeRequest>(shared_from_this(), "read_script");
        ...
        read_goal_.voice_name = voice_name_;
...
int main(int argc, char **argv) {
    ...
    auto parameter_node = std::make_shared<rclcpp::Node>("notify_automated_deep_fake_parameter_node");
    auto param_listener = std::make_shared<shr_plan_parameters::ParamListener>(parameter_node);
    auto params = param_listener->get_params();


    std::vector<std::shared_ptr<NotifyAutomatedDeepFake>> all_nodes;
    for (auto i = 0ul; i < params.notify_automated_deep_fake_actions.actions.size(); i++) {
        auto action = params.notify_automated_deep_fake_actions.actions[i];
        auto script_name = params.notify_automated_deep_fake_actions.script_names[i];
        auto wait_time = params.notify_automated_deep_fake_actions.wait_times[i];
        auto sensor_topic = params.notify_automated_deep_fake_actions.topics[i];
        auto voice_name = params.notify_automated_deep_fake_actions.voice_names[i];
        auto ind = all_nodes.size();
        all_nodes.push_back(std::make_shared<NotifyAutomatedDeepFake>(action, script_name, wait_time, sensor_topic, voice_name));
