...
namespace planning_controller {
  ...
  public:
    PlanningController()
        : rclcpp::Node("planning_controller"), state_(IDLE) {
...
int main(int argc, char **argv) {
  ...
  auto spin_node = std::make_shared<planning_controller::PlanningControllerSpin>();
    ...
    if (node.active_protocol == "medicine_reminder") {
        domain_node->set_parameter({"model_file", pkgpath + "/pddl/food_domain.pddl"});
        problem_node->set_parameter({"model_file", pkgpath + "/pddl/food_problem.pddl"});
    }
    if (node.active_protocol == "food_reminder") {
        domain_node->set_parameter({"model_file", pkgpath + "/pddl/medicine_domain.pddl"});
        problem_node->set_parameter({"model_file", pkgpath + "/pddl/medicine_problem.pddl"});
    }
    if (node.active_protocol.empty() ) {
        domain_node->set_parameter({"model_file", pkgpath + "/pddl/medicine_domain.pddl"});
        problem_node->set_parameter({"model_file", pkgpath + "/pddl/medicine_problem.pddl"});
    }
    ...
    exe.add_node(spin_node->get_node_base_interface());
    exe.spin();
    rclcpp::shutdown();
    return 0;
}